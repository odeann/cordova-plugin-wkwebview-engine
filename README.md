...
The copy of apache/cordova-wkwebview-engine with the only fix applied: it makes the plugin to obey the `KeyboardDisplayRequiresUserAction` option.

Just add `<preference name="KeyboardDisplayRequiresUserAction" value="false" />` to your `config.xml` to be able to select inputs from your javascripts.
